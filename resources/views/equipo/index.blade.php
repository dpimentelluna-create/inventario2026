                                ...
                                <!-- MODIFICACIONES ENCABEZADOS -->
                                    ...
                                    <tbody>
                                        @foreach ($equipos as $i => $equipo)
                                            ...
                                            <td>{{ $equipo->marca }}</td>
                                            <td>{{ $equipo->modelo }}</td>
                                            <!--<td class = "text-center">{{ $equipo->codigo_inventario }}</td>-->
                                            ...
                                            <td>{{ $equipo->observacion }}</td>
                                                ...
                                                <td  class="text-center columna-acciones">
                                                    ...
                                                    <form action="{{ route('equipos.destroy', $equipo->id) }}" method="POST">
                                                        <a class="btn btn-info btn-accion " href="{{ route('equipos.show', $equipo->id) }}"><i class="fa-solid fa-eye"></i> {{ __('') }}</a>
                                                        <a class="btn btn-warning btn-accion" href="{{ route('equipos.edit', $equipo->id) }}"><i class="fa-solid fa-pen-to-square"></i> {{ __('') }}</a>
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger btn-accion" onclick="event.preventDefault(); confirm('¿Estás seguro de eliminar?') ? this.closest('form').submit() : false;"><i class="fa-solid fa-trash"></i> {{ __('') }}</button>
                                                    </form>
                                                    </div>
                                                </td>
                                            </tr>
